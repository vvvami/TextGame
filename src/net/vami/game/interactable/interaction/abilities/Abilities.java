package net.vami.game.interactable.interaction.abilities;

public final class Abilities {
    public static final FlamesAbility FLAMES = new FlamesAbility();
    public static final HypnosisAbility HYPNOSIS = new HypnosisAbility();
    public static final PrayAbility PRAY = new PrayAbility();
    public static final RageAbility RAGE = new RageAbility();
    public static final SearchAbility SEARCH = new SearchAbility();
    public static final SummonAbility SUMMON = new SummonAbility();
}
